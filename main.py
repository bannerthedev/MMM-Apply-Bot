...
intents.guilds = True
intents.message_content = True  # needed for reading DM replies in some libs

bot = commands.Bot(command_prefix="!", intents=intents)

# Helper view for the initial select
...
class RegisterTypeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await start_application_flow(interaction.user, self.values[0], interaction)
...
async def start_application_flow(user: discord.User, app_type: str, interaction: discord.Interaction):
    ...
    # utility to collect text answer (required)
    async def collect_text(question):
        await dm.send(question)
        def check(m: discord.Message):
            return m.author.id == user.id and isinstance(m.channel, discord.DMChannel)
        try:
            msg = await bot.wait_for('message', timeout=300.0, check=check)  # 5 minutes
    ...
    # utility to ask yes/no via select
    async def ask_yes_no(question):
        ...
        if view.value is None:
            await dm.send("Timed out. Please re-run /register.")
            raise asyncio.TimeoutError()
        return view.value
    ...
    try:
        ...
        # Prepare embed for application channel
        embed = discord.Embed(
        ...
        # Prepare embed (keep your existing embed code above this)

        # Staff view (defined inside coroutine so it can access variables)
        class StaffDecisionView(View):
            ...
            async def accept(self, interaction: discord.Interaction, button: Button):
                ...
                await interaction.response.edit_message(content=f"Application accepted by {staff_member.display_name}.", embed=interaction.message.embeds[0], view=None)
                ...
                # assign role in the guild where the interaction happened (fallback to configured GUILD_IDS)
                try:
                    guild = interaction.guild or (await bot.fetch_guild(GUILD_IDS) if isinstance(GUILD_IDS, int) else None)
                    if guild:
                        role_id = None
            ...
            async def deny(self, interaction: discord.Interaction, button: Button):
                ...
                await interaction.response.edit_message(content=f"Application denied by {staff_member.display_name}.", embed=interaction.message.embeds[0], view=None)
                try:
                    applicant = await bot.fetch_user(self.target_user_id)
        ...
        # timed out or early exit; a timeout message was already sent
        return
...
async def on_ready():
    ...
    try:
        ...
        # determine guild object (supports None, int, or list)
        guild_obj = None
